...
export const ChatSideNavProfile = ({ character, active, onClick }: { character: { profile: string, summary: string, description: string, name: string }, active?: boolean, onClick?: MouseEventHandler<HTMLButtonElement> }) => {
    ...
    return (
        <button onClick={onClick} className={ (active?"bg-white": "") + " flex flex-row items-center hover:bg-white rounded-sm p-2 border-b py-6"}>
            ...
            <div className="w-4/5 text-left ml-4">
                ...
                <div className="ml-2 text-[.75rem] mt-2">{character.summary}</div>
            </div>
        </button>
